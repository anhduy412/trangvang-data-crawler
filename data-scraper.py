...
for page in range(1, 3):
    ...
    for company in companies:
        ...
        # Extract address
        address_div = company.find('div', class_='logo_congty_diachi')
        if address_div:
            address = address_div.find_all('div')[1].get_text(strip=True) if len(address_div.find_all('div')) > 1 else ''
        else:
            address_div = company.find('div', class_='listing_diachi_nologo')
            if address_div:
                address = address_div.find_all('div')[1].get_text(strip=True) if len(address_div.find_all('div')) > 1 else ''
            else:
                address = ''
        ...
        phone_div = company.find('div', class_='listing_dienthoai')
        if phone_div:
            phone = phone_div.find('a').get_text(strip=True) if phone_div.find('a') else ''
        else:
            # If not found, check the second div
            phone_div = company.find('div', class_='p-2 pt-0 ps-0 pe-4 pb-0')
            if phone_div:
                phone = phone_div.find('a').get_text(strip=True) if phone_div.find('a') else ''
            else:
                phone = ''
